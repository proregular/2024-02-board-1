...
/*
    ...
    데이터를 보낼 때 보여지나 안보여지나 차이가 있음
    1. 쿼리스트링 방식 (파라미터라고 부르기도 함), url에 데이터를 포함하는 방식
    2. body에 담아서 보내는 방식(FormData, JSON)

    쿼리스트링 모양: url + 쿼리스트링 (?로 시작 key=value, 여러개하면 & 구분)
                  www.naver.com?name=홍길동&age=12&height=172.1

    대용량의 데이터를 보내야할 때도 body에 데이터를 담아서 보낸다. url은 결국 길이제한이 있기 때문에
    ...
    위 작업은 2가지 밖에 없었기 때문에 같은 주소값으로 metho를 구분할 수 있었다.
    ...
    (post) /board-create -  게시판 글 등록하는 작업 처리
    (get) /board_modify - 게시판 글 수정하는 화면
    (post) /board_modify - 게시판 글 수정하는 작업 처리
    (get/post) /board-delete - 게시판 글 삭제하는 작업 처리

    첫 페이지(index)를 띄울 때 소프트웨어(FE 작업 코드)가 모두 다운로드 됨.
    ...
    (요청의 method 크게 4가지로 나누어진다.)
    - POST 방식: Create - Insert 작업
    - GET 방식: Read - Select 작업
    - PUT / PATCH 방식: Update - Update 작업
    - DELETE 방식: Delete - Delete 작업
...
/*
  final 붙은 맴버필드 DI받을 수 있게 생성자를 만든다.
  어노테이션 안붙이면 직접 생성자를 만들면 된다.
 */
@RequiredArgsConstructor
@RestController // 빈등록 + 컨트롤러 임명, 빈등록은 스프링컨테이너가 직접 객체화를 한다.
